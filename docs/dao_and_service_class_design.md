...
| BlogChunkDAO, BlogChunkService | blog/ | dao.py, service.py |
...
| SearchDAO, SearchService | search/ | dao.py, service.py |
...
class IBlogDAO(ABC):
    ...
    @abstractmethod
    def list_blogs(self, source_id: uuid.UUID | None, tag_id: uuid.UUID | None, keyword: str | None, page: int, count: int): ...

    @abstractmethod
    def count_blogs(self, source_id: uuid.UUID | None, tag_id: uuid.UUID | None, keyword: str | None): ...
...
class ISummaryDAO(ABC):
    @abstractmethod
    def get_by_blog_id(self, blog_id: str): ...
...
```python
class IBlogChunkDAO(ABC):
    @abstractmethod
    def get_by_blog_id(self, blog_id: str): ...

    @abstractmethod
    def create(self, blog_id: str, chunk_text: str, embedding: list[float]): ...


class BlogChunkDAO(IBlogChunkDAO):
    def __init__(self, db: Session): ...
    def get_by_blog_id(self, blog_id: str): ...
    def create(self, blog_id: str, chunk_text: str, embedding: list[float]): ...
```
...
class IPrerequisiteDAO(ABC):
    @abstractmethod
    def get_by_topic_name(self, topic_name: str): ...
...
```python
class ISearchDAO(ABC):
    @abstractmethod
    def keyword_search(self, query: str) -> list[str]: ...

    @abstractmethod
    def vector_search(self, query_embedding: list[float]) -> list[str]: ...
...
- `keyword_search` — tsvector search against `blog` table. Returns ordered list of `blog_id`s. Excludes limited tier articles (`word_count < 150`).
- `vector_search` — cosine similarity search against `blog_chunk` table via pgvector. Maps chunk matches back to `blog_id`s. Returns ordered list of `blog_id`s.
- Both use raw SQL via `text()` — ORM is insufficient for these query shapes.
- Result set capped at `SEARCH_RESULT_LIMIT` (defined in `constants.py`).
...
class BlogService:
    ...
    def get_blog_by_blog_id(self, blog_id: str): ...
...
class SummaryService:
    ...
    def get_summary_by_blog_id(self, blog_id: str): ...
...
class SimplifyService:
    ...
    def get_simplify_by_blog_id(self, blog_id: str): ...
...
```python
class BlogChunkService:
    def __init__(self, dao: IBlogChunkDAO): ...
    def get_chunks_by_blog_id(self, blog_id: str): ...
    def create_chunk(self, blog_id: str, chunk_text: str, embedding: list[float]): ...
```
...
class PrerequisiteService:
    ...
    def get_prerequisite_by_topic_name(self, topic_name: str): ...
...
```python
class SearchService:
    def __init__(self, dao: ISearchDAO): ...
    def keyword_search(self, query: str) -> list[str]: ...
    def vector_search(self, query_embedding: list[float]) -> list[str]: ...
```

- Thin wrappers over `SearchDAO`. No business logic.
- RRF fusion and query embedding are handled in `BlogHandler`, not here.

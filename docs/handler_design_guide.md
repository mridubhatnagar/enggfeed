...
3. After all sources are processed (success or failure): `_check_daily_budget()`, then `_check_monthly_budget()`

**Private helpers:**
- `_fetch_thumbnail(link)` — scrapes og:image, returns url or None
- `_check_daily_budget()` — sums today's `llm_usage.cost_usd` via `llm_usage_service.get_daily_costs(day_start)`. If total exceeds `settings.LLM_DAILY_COST_ALERT_USD` (env `LLM_DAILY_COST_ALERT_USD`, default `3.00`), logs a warning, calls `sentry_sdk.capture_message(...)` (short one-line summary), and calls `utils.send_alert_email(...)` with subject `"[enggfeed] Daily LLM spend $X.XX — over $Y.YY threshold (YYYY-MM-DD)"` and a body built from the summary line plus `_budget_snapshot_lines(now)`. Exists to catch a single runaway ingest run (e.g. a new source's full RSS backlog) same-day, rather than waiting for the monthly total to cross its own threshold.
- `_check_monthly_budget()` — sums this calendar month's `llm_usage.cost_usd` via `llm_usage_service.get_monthly_costs(month_start)`. If total exceeds `settings.LLM_MONTHLY_COST_ALERT_USD` (env `LLM_MONTHLY_COST_ALERT_USD`, default `10.00`), logs a warning, calls `sentry_sdk.capture_message(...)`, and calls `utils.send_alert_email(...)` with the analogous monthly subject/body.
- `_budget_snapshot_lines(now)` — shared by both checks. Returns 3 lines: today's `(calls, cost)` via `get_daily_costs`, this month's via `get_monthly_costs`, and all-time-till-date via `get_total_costs()` (unfiltered aggregate over `llm_usage`). Gives the email body full context beyond just the one threshold that tripped.
- `utils.send_alert_email(subject, body)` — plain-text SMTP email. Reads `SMTP_HOST`/`SMTP_PORT`/`SMTP_USERNAME`/`SMTP_PASSWORD`/`SMTP_FROM_EMAIL`/`ALERT_EMAIL_TO` from `settings`; no-ops with a warning log (never raises) if SMTP isn't configured.
- Both budget checks are alert-only by design (explicitly decided over auto-pausing ingest) — each wrapped in its own try/except so a failure is logged/captured but never aborts the ingest job, run once per ingest job (daily cadence via GitHub Actions cron), and do **not** cover spend from one-off scripts (`backfill_*.py`) that bypass `LLMUsageService.create_llm_usage`. Added after an incident where LLM credits ran out mid-month with no visibility.

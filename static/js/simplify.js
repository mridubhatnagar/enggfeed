    function simplifyPage() {
      return {
        ...
        async _load() {
          ...
          try {
            const res  = await fetch(`/api/v1/blogs/${encodeURIComponent(this.blogId)}/simplify`, { credentials: 'include' });
            const json = await res.json();
            if (json.success && json.data) {
              this.simplifyDetail = json.data;
            } else if (json.error?.code === 404) {
              this.flashMessage = "Simplify isn't available for this article yet. Please check back later.";
